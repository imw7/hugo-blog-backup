...
`Gin` 是一个用 `Go` 语言编写的 `Web` 框架。它是一个类似于 `martini` 但拥有更好性能的 `API` 框架, 由于使用了 `httprouter`，速度提高了近40倍。 如果你是性能和高效的追求者, 你会爱上 `Gin`。<!--more-->
...
浏览器输入 `http:127.0.0.1/test?name` 即可在终端看到类似以下的输出：
