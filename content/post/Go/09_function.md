...
函数是组织好的、可重复使用的、用于执行指定任务的代码块。本文介绍了`Go`语言中函数的相关内容。<!--more-->
...
接下来我们来看一下语句块定义的变量，通常我们会在if条件判断、`for`循环、`switch`语句上使用这种定义变量的方式。
...
上面语句定义了一个`calculation`类型，它是一种函数类型，这种函数接收两个`int`类型的参数并且返回一个int类型的返回值。
...
![defer执行时机](https://blog.imw7.com/images/Go/function/defer.png)
