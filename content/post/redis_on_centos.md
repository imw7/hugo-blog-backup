...
toc: true
date: 2022-01-12 12:05:22
---

本文介绍如何在centos上安装、配置、启动、关闭和卸载redis。<!--more-->
...
1. 在redis官网下载最新版本的[redis-6.2.6](https://download.redis.io/releases/redis-6.2.6.tar.gz)。
...
由于 `Redis` 是由`C 语言`写成的，所以需要 `gcc` 编译工具。
...
如果出现类似下面的输出，则说明`gcc`已经正确安装在centos服务器上了。
...
安装包中都是C语言编写的源程序，需要使用`make`命令进行编译。
...
**注意**：`PREFIX` 必须大写，同时会自动为我们生成redis目录，并将结果安装到此目录。
...
1. 先把`redis`服务关闭
2. 再把`/usr/local/redis/bin/`目录下的redis相关的文件删除即可。
